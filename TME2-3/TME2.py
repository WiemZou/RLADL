...
#gamma=1 toutes politiques ont le meme coup meme interet quel que soit le chemin choisi alors que pour gamma inferieur a 1 on force a choisir le plus court
...
def PolicyIteration(env,eps,gamma):
    states, P = env.getMDP()
    #pi et V de dimension P nombre d'etats non terminaux car terminaux par defaut c'est 0 on ne peut pas en sortir
    states_P=list(P.keys())
    pi0=dict()
    for s in states_P:
...
#            print(V1)
        ...
        print('break')
        pi1 = EvalPolicy(states,P,V0,gamma)
            ...
            for a in P[s]:
                for dest in P[s][a]:
                    ...
                    if d not in states_P:
                        ...
                        d=states_P.index(d)
                        act[a] += p*(r + gamma*V0[d])
            V1[si] =  np.max(act)
        i = i + 1 
        print(np.linalg.norm(V1-V0,ord=np.inf))
        if np.linalg.norm(V1-V0,ord=np.inf)<= eps:
            V0 = np.copy(V1)
            break
        V0 = np.copy(V1) 
        
    pi = EvalPolicy(states,P,V0,gamma)
    print('end')
...
#    envm = wrappers.Monitor(env, directory=outdir, force=True, video_callable=False)
    ...
    i=0
    gamma=0.75
    agent = Agent(env,eps,gamma,ValueIteration)
    while True:
...
#plot des rewards en fonction des epoques, nb d'actions au fil du temps
        
 #politique dictionnaire ou chaque etat dis ou on doit aller
 
# V value : dictionnaire
# Fonction qui fait policy iteration p        act = np.zeros(len(P[s])) # erreur avec etats terminaux a voir  rend MDP